...
public class MappingProfiles : Profile
{
    public MappingProfiles()
    {
        ...
        CreateMap<School, GetLessonsBySchoolIdResponse>()
            .ForMember(s => s.LessonName, opt => opt.MapFrom(s => s.SchoolClasses.Select(s => s.SchoolClassLessons.Select(s => s.Lesson.Name).ToList())))
            .ForMember(s => s.SchoolName, opt => opt.MapFrom(s => s.Name))
            .ReverseMap();
